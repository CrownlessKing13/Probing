...
public class quadraticProbing{
   ...
   public static int total=0;
    ...
    public long probe(long uninum,String key){
        ...
        if (table[hashnum] == null){
            ...
            f=f+(f<<1)-1;
            hashnum = hashnum=Integer.parseInt((uninum+f)%tableSize+"");
            if (table[hashnum] == null){
            return hashnum;
            }
   ...
   public void insert(String file,int Size){
        ...
        System.out.println("Sucessfully stored in table with QuadraticHashing structure");
   ...
   while(true){
     f=f+(f<<1)-1;
     start = Integer.parseInt((hash(key)+f)%tableSize+"");
     countSearch++;
     if(countSearch>tableSize){return "Date/Time is not found";}
     if(table[start]==null){
    return "Date/Time is not found";
    }
     if(table[start].split(",")[0].equals(key)){
